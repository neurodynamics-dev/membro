...
| `treinamentos-conteudo.mjs` | os treinamentos escritos em `treinamentos/`: cada `.md` lido sem aviso pelo leitor do portal, só pendência marcada impedindo publicar, todo `arquivo:` no rol da 21.0 e todo `treinamento:` entre eles, a abertura "Ao fim deste módulo, você…", explicação em toda questão, um roteiro para cada vídeo marcado (e nada a mais) e a semente `db/v25_treinamentos_iniciais.sql` em dia com os `.md` — sai com código 1 se algo falhar |

```bash
node colisoes.mjs                       # não precisa de servidor nem de npm install
node treinamentos-conteudo.mjs          # também não
