...
| `v25_treinamentos_iniciais.sql` | os seis primeiros treinamentos (NRO-TRE-001 a 006: introdução ao SOMA, tempo e agenda, ISO 9001 e Arquivos, redes sociais, confidencialidade, gestão de projetos), em **rascunho** — nada publicado nem atribuído. É gerada dos `.md` de [`treinamentos/`](../treinamentos/LEIAME.md) por `treinamentos/gerar-semente.mjs`; não edite à mão |
...
A **25.0** põe os seis primeiros treinamentos no banco, em rascunho. Um
número que já existe fica como está — o treinamento, o rascunho e o que
alguém já editou nele —, e a tabela do fim diz isso. Os vídeos ainda não
gravados estão marcados no texto (`[VÍDEO A GRAVAR: …]`), com o roteiro de
cada um em [`treinamentos/roteiros/`](../treinamentos/roteiros/LEIAME.md), e o
portal não publica enquanto houver marca.
...
# 25.0, num banco novo com a 24.0 e ainda sem a 25.0: o teste prepara um
# NRO-TRE-003 que a semente precisa respeitar e roda a 25.0 três vezes
createdb t25
psql -d t25 -f testes/esqueleto.sql -f testes/esqueleto_storage.sql
psql -d t25 -f v15_atividades.sql -f v16_pessoal.sql -f v17_grupos_acesso.sql \
            -f v18_teste_email.sql -f v19_grupos_hierarquia.sql -f v24_treinamentos.sql
psql -d t25 -f testes/v25_treinamentos_iniciais.sql   # 59 asserções (roda a 25.0 três vezes; publica um e o faz até o certificado)
```
